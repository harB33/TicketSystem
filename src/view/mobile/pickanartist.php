...
<div class="flex flex-col w-full h-full relative overflow-y-auto custom-scrollbar-v">
    ...
    <form method="POST" action="?page=pickanarena" class="w-full flex flex-col h-full relative">
        ...
        <div id="artists-grid-mobile" class="grid grid-cols-3 gap-4 p-4 z-10 pb-40">
            ...
            <?php foreach($initial_artists as $artist): ?>
                <label class="relative border border-white/10 rounded-xl aspect-square overflow-hidden cursor-pointer select-none bg-white/5 block hover:border-primary/50 transition-colors animate-fade-in">
                    ...
                    <input type="checkbox" name="artists[]" id="chk_mobile_<?= $artist['artist_id'] ?>" value="<?= $artist['artist_id'] ?>" class="sr-only peer" onclick="handleArtistClickMobile(<?= $artist['artist_id'] ?>, '<?= htmlspecialchars(addslashes($artist['genre'])) ?>')" <?php echo (isset($_POST['artists']) && in_array($artist['artist_id'], $_POST['artists'])) ? 'checked' : ''; ?>>
...
<script>
    const poolArtistsMobile = <?php echo json_encode($pool_artists); ?>;
    const gridMobile = document.getElementById('artists-grid-mobile');

    function escapeHtmlMobile(unsafe) {
    ...
    function handleArtistClickMobile(artistId, genre) {
        const checkbox = document.getElementById('chk_mobile_' + artistId);
        console.log('handleArtistClickMobile', artistId, checkbox ? checkbox.checked : null, genre);
        ...
        for (let i = 0; i < poolArtistsMobile.length; i++) {
            const poolGenreStr = (poolArtistsMobile[i].genre || '').toLowerCase();
            const isMatch = targetGenres.some(g => poolGenreStr.includes(g));

            if (isMatch) {
                matches.push(poolArtistsMobile[i]);
                poolArtistsMobile.splice(i, 1);
                i--;
        ...
        while (matches.length < 3 && poolArtistsMobile.length > 0) {
            matches.push(poolArtistsMobile.shift());
        }
        ...
        matches.forEach((artist, index) => {
            setTimeout(() => {
                // If this artist checkbox already exists in the DOM, skip inserting a duplicate
                if (document.getElementById('chk_mobile_' + artist.artist_id)) return;
                ...
                const imgHtml = artist.image_url 
                    ? `<img src="${escapeHtmlMobile(artist.image_url)}" class="w-full h-full object-cover">` 
                ...
                const safeGenre = escapeHtmlMobile(artist.genre || '');
                
                div.innerHTML = `
                    ${imgHtml}
                    <div class="absolute bottom-0 w-full bg-linear-to-t from-black/90 to-transparent p-2 text-center">
                        <span class="text-white text-xs font-bold truncate block">${escapeHtmlMobile(artist.name)}</span>
                    </div>
                    <input type="checkbox" name="artists[]" id="chk_mobile_${artist.artist_id}" value="${artist.artist_id}" class="sr-only peer" onclick="handleArtistClickMobile(${artist.artist_id}, '${safeGenre.replace(/'/g, "\\'")}')">
                    <div class="absolute inset-0 bg-primary opacity-0 peer-checked:opacity-50 transition-opacity duration-200"></div>
                    <div class="absolute inset-0 opacity-0 peer-checked:opacity-100 flex items-center justify-center transition-opacity duration-200">
                        <div class="w-14 h-14 border-4 border-white rounded-full flex items-center justify-center z-10">
                ...
                if (insertAfterNode && insertAfterNode.parentNode) {
                    ...
                    gridMobile.appendChild(div); // Fallback
                }
